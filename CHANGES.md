...
- [CHANGE] JSON Schema 検証を shiguredo_jesse から jsone の JSON Schema 実装に移行する
  - `jsone_schema` を直接使う
  - 400 応答の `error_reason` の `schema` から `$id` が消える
  - JSON として不正な Body の `error_type` が InvalidJSON から MalformedJSON に変わる
...
- [UPDATE] gun のバージョンを 2.4.1 に上げる
  - @voluntas
- [UPDATE] cowboy のバージョンを 2.16.1 に上げる
  - @voluntas
- [UPDATE] cowboy の cowlib 依存バージョンを 2.17.1 に上書きする
  - <https://github.com/ninenines/cowboy/issues/1670#issuecomment-2670967044>
  - @voluntas
...
- [UPDATE] cowboy のバージョンを 2.13.0 に上げる
  ...
  - <https://github.com/ninenines/cowboy/issues/1670#issuecomment-2670967044>
  - @voluntas
- [UPDATE] GitHub Actions の docker の OTP を shiguredo 27.2.4 / OpenSSL 3.4.1 に上げる
  - @voluntas
